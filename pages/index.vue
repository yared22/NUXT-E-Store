<template>
    <div class="max-w-2xl mx-auto my-10 p-5 bg-gray-50 rounded-lg shadow-lg font-sans text-gray-700">
        <h1 class="text-gray-800 font-bold mb-5 text-3xl">
            Home
        </h1>
        <p class="text-lg leading-relaxed mb-5">
            Building dreams with every line of code. <br />
            Where logic meets creativity in perfect harmony.
        </p>
        <NuxtLink to="/about" class="text-blue-500 no-underline font-semibold transition-colors duration-300 hover:text-blue-700 hover:underline">About</NuxtLink>
    </div>
</template>
